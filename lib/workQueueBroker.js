...
WorkQueueBroker = (function(_super) {
  ...
  WorkQueueBroker.prototype.initEmitters_ = function() {
    ...
    return this.qmgr.on('message', function(queueName, payload) {
      if (_this.isValidQueueName(queueName) && _this.queues[queueName].onJob) {
        if (_this.queues[queueName].onJob(payload)) {
          return _this.qmgr.pop(queueName);
        }
      }
    });
  };
  ...
  WorkQueueBroker.prototype.begin = function() {
    var queueName, _i, _len, _ref, _results;
    _ref = Object.keys(this.queues);
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      queueName = _ref[_i];
      _results.push(this.qmgr.pop(queueName));
    }
    return _results;
  };

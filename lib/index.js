...
RedisQueue = (function(_super) {
  ...
  function RedisQueue(client, timeout) {
    var _this = this;
    this.client = client;
  ...
  RedisQueue.prototype.monitor = function() {
    ...
    return (_ref = this.client).brpop.apply(_ref, __slice.call(keysToMonitor).concat([this.timeout], [function(err, replies) {
      if (err != null) {
      ...
      if (!_this.stop) {
        return _this.monitor.apply(_this, __slice.call(keysToMonitor).concat([_this.timeout]));
      }
    }]));
  };
